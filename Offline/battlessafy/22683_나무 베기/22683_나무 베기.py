...
#                 q.append((nx, ny, dir, moves + 1, cnt, path + [(nx, ny), 'A']))     # 이동 거리 1증가 후, 다음 작업 대상 추가
#
...
#                 q.append((nx, ny, dir, moves + 1, cnt - 1, path + [(nx, ny), 'A']))     # 이동 거리 1증가, 나무를 자른 횟수 1 감소
#
#         # 왼쪽으로 회전
#         nd = (dir - 1) % 4
    ...
    while q:
        ...
        for d in range(4):
            ...
            if 0 <= nx < N and 0 <= ny < N:
                if [nx, ny] == target_position:
                    path.append((dir_dict[d], (nx, ny), 'F'))
                    return path

                if map_data[nx][ny] == 'G' and not visited[nx][ny]:
                    visited[nx][ny] = 1
                    q.append((nx, ny, path + [(dir_dict[d], (nx, ny), 'A')]))
...
for tc in range(1, T + 1):
    ...
    result = bfs(my_position, target_position)
    for direction, position, action in result:
